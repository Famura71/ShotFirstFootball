...
"""
Shot-First Football - D-Zone 1 Multi-Season StatsBomb xG Calculator
...
Run:
    python sff_statsbomb_dzone1_multi_season.py
"""
...
COMPETITION_SEASONS: List[Tuple[int, int]] = [
    ...
    (11, 4),   # Uncomment if available in your local test
    (11, 1),   # Uncomment if available
...
def plot_dzone1_shots(df: pd.DataFrame, output_path="sff_dzone1_shots.png"):
    radius = zone1_radius()
    ...
    goals = dzone1[dzone1["outcome"] == "Goal"]
    non_goals = dzone1[dzone1["outcome"] != "Goal"]
    ...
    ax.plot(xs, ys, color="#00cc55", linewidth=2.5, label="Zone 1 boundary")

    ax.text(96, 47, "D-Zone 1", color="#00cc55", fontsize=16, weight="bold")
...
def summarize(df: pd.DataFrame) -> None:
    ...
    dzone1 = df[df["is_dzone1"]]
    outside = df[~df["is_dzone1"]]

    print("\n--- Shot-First Football D-Zone 1 Multi-Season Summary ---")
    ...
    print(f"D-Zone 1 shots: {len(dzone1)}")
    print(f"Outside D-Zone 1 shots: {len(outside)}")

    if len(dzone1) > 0:
        print(f"D-Zone 1 average xG: {dzone1['xg'].mean():.4f}")
        print(f"D-Zone 1 median xG: {dzone1['xg'].median():.4f}")
        print(f"D-Zone 1 goal rate: {(dzone1['outcome'] == 'Goal').mean():.4f}")

    if len(outside) > 0:
        print(f"Outside D-Zone 1 average xG: {outside['xg'].mean():.4f}")
        print(f"Outside D-Zone 1 median xG: {outside['xg'].median():.4f}")
        print(f"Outside D-Zone 1 goal rate: {(outside['outcome'] == 'Goal').mean():.4f}")

    if len(dzone1) > 0:
        print("\nTop D-Zone 1 shooters by shot count:")
        summary = dzone1.groupby("player").agg(
            shots=("xg", "count"),
            avg_xg=("xg", "mean"),
    ...
    print(df.groupby(["competition_id", "season_id"]).agg(
...
def main():
    ...
    csv_path = "sff_statsbomb_dzone1.csv"
    df.to_csv(csv_path, index=False)
    print(f"\nSaved data: {csv_path}")

    plot_dzone1_shots(df)
