...
# ------------------------------------------------------------
# Shot-First Football pitch template
# Vertical black pitch + Zone 0 and Zone 1
# Zone 0: circle passes through penalty arc endpoint
# Zone 1: circle passes through penalty area corners
# Drawn for both top and bottom goals
# ------------------------------------------------------------
...
def draw_zone0(ax, penalty_spot, is_top=True):
    """
    Zone 0:
    Goal-centered semicircle passing through a penalty arc endpoint.
    """
...
def draw_zone1(ax, penalty_x, penalty_width, penalty_depth, is_top=True):
    """
    Zone 1:
    Goal-centered semicircle passing through the two corners of the penalty area.
    """
    origin_x = PITCH_WIDTH / 2
    ...
    right_corner = (penalty_x + penalty_width, corner_y)

    # Both corners are symmetric, so using either one is enough
    ...
    ax.text(origin_x + 12, label_y, "Zone 1", color=ZONE1_COLOR,
            fontsize=13, fontweight="bold", va="center", ha="left")
