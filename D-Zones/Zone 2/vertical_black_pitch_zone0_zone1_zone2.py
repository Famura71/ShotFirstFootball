...
def draw_zone2(ax, is_top=True):
    """
    Zone 2 circle passes through the corner points.

    Top side:
        Origin: top goal center (PITCH_WIDTH / 2, PITCH_LENGTH)
        Corner points: (0, PITCH_LENGTH) and (PITCH_WIDTH, PITCH_LENGTH)

    Bottom side:
        Origin: bottom goal center (PITCH_WIDTH / 2, 0)
        Corner points: (0, 0) and (PITCH_WIDTH, 0)
    """
    origin_x = PITCH_WIDTH / 2

    # Since the circle passes through both corner points,
    # its radius is half of the pitch width.
    zone_radius = PITCH_WIDTH / 2

    if is_top:
        origin_y = PITCH_LENGTH
        theta = np.linspace(np.pi, 2 * np.pi, 500)
        label_y = PITCH_LENGTH - zone_radius + 4
    else:
        origin_y = 0
        theta = np.linspace(0, np.pi, 500)
        label_y = zone_radius - 4

    x = origin_x + zone_radius * np.cos(theta)
    y = origin_y + zone_radius * np.sin(theta)

    ax.plot(x, y, color=ZONE2_COLOR, linewidth=2.5)
    ax.text(origin_x + 6, label_y, "Zone 2", color=ZONE2_COLOR,
            fontsize=13, fontweight="bold", va="center", ha="left")
...
def main():
    ...
    plt.tight_layout()
    plt.savefig("vertical_black_pitch_zone0_zone1_zone2.png",
